use infrastructure::{DynamoDbClient, OptimisticLockService, EventRepository, ProjectionRepository};
use shared::Config;
use domain::{TodoEvent, TodoId, Todo, TodoError};
use std::sync::Arc;
...
async fn test_optimistic_lock_basic_operations() {
    ...
    match service.projection_repo.save_projection(&family_id, initial_todo.clone()).await {
        Ok(_) => {
            println!("✓ 初期ToDo保存成功");
            ...
            match service.update_todo_with_lock(
                &family_id,
                &todo_id,
    ...
    match service.projection_repo.save_projection(&family_id, initial_todo.clone()).await {
        Ok(_) => {
            ...
            let handle3 = tokio::spawn(async move {
                ...
                service3.update_todo_with_retry(
                    &family_id3,
                    &todo_id3,
                    5,
                    |todo| {
            ...
            // 最終状態を確認
            if let Ok(Some(final_todo)) = service.projection_repo.get_projection(&family_id, &todo_id).await {
                println!("✓ 最終ToDo状態: version={}, title={}, completed={}", 
...
async fn test_event_projection_consistency() {
    ...
    match event_repo.save_event(&family_id, events[0].clone()).await {
        Ok(_) => {
            ...
            let _ = projection_repo.save_projection(&family_id, current_todo.clone()).await;
            ...
            if let Ok(stored_events) = event_repo.get_events(&family_id, &todo_id).await {
                if !stored_events.is_empty() {
                    ...
                    if let Ok(Some(projection_todo)) = projection_repo.get_projection(&family_id, &todo_id).await {
...
async fn test_error_handling_and_retry() {
    ...
    if service.projection_repo.save_projection(&family_id, initial_todo.clone()).await.is_ok() {
        // 無効なタイトル（空文字）での更新を試行
        match service.update_todo_with_retry(
            &family_id,
            &todo_id,
            3,
            |_todo| Err(TodoError::Validation("無効なデータ".to_string())),
        ).await {
...
async fn test_performance_basic() {
    ...
    for i in 0..todo_count {
        ...
        if service.projection_repo.save_projection(&family_id, todo).await.is_ok() {
    ...
    if !todo_ids.is_empty() {
        ...
        for (i, todo_id) in todo_ids.iter().enumerate() {
            ...
            let handle = tokio::spawn(async move {
                service_clone.update_todo_with_retry(
                    &family_id_clone,
                    &todo_id_clone,
                    3,
                    |todo| {
        ...
        if success_count > 0 {
            ...
            assert!(avg_time_per_operation < Duration::from_secs(5), 
                   "平均処理時間が5秒を超えています: {:?}", avg_time_per_operation);
        }
    } else {
